...
vector<int> matchingStrings(vector<string> stringList, vector<string> queries) {
	...
	for(query = queries.begin();query<queries.end();query++){
		...
		for(sourceString = stringList.begin();sourceString<stringList.end();sourceString++){
			for(i=0;i<query.size();){
				int matchStartIndex = INT_MIN;
				for(j=0;j<sourceString.size();){
					if(sourceString[j]==query[i]){
						i++;
						if(matchStartIndex==INT_MIN){
							matchStartIndex = 
						}
					}
				}
